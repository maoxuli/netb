...
bool StreamReader::SerializeString(std::string& s, const std::string& delim)
{
    if(mStream == NULL) return false;
    if(mStream->Readable() < delim.length()) return false;

    ssize_t n = mStream->Readable(delim.c_str());
    if(n < 0) return false;
    if(n == 0) return true;
    
    const unsigned char* p = (const unsigned char*)mStream->Read();
    std::string(p, p + n).swap(s);
    mStream->Read(n + delim.length());
    return true;
}
