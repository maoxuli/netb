...
#ifndef NET_BASE_SOCKET_ADDRESS_H
#define NET_BASE_SOCKET_ADDRESS_H
...
public: 
	...
	// Constructor, the family of address is given
	SocketAddress(sa_family_t family) throw();

	// Constructor, given host and port: 127.0.0.1:9000
	SocketAddress(const char* addr, sa_family_t family = AF_INET);
...
public:
	...
	void Reset(sa_family_t family = AF_UNSPEC) 
	{ 
		memset(this, 0, sizeof(struct sockaddr_storage)); 
		ss_family = family; 
	}

	// Empty address is ss_family == AF_UNSPEC
	...
	void Set(const char* host, unsigned short port);

	// Check special address
	bool Any() const; // 0.0.0.0:0
	bool AnyPort() const; // port = 0
	bool AnyHost() const; // 0.0.0.0
	bool Localhost() const; // 127.0.0.1
	bool Loopback() const; // 127.0.0.0/8
